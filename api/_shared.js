// =====================================================================
// Shared helpers for the Biome Quest standalone backend.
...
// test -> live switch needs no code change). Biome-Quest-specific bits:
// BIOME- code prefix, bq_owners/bq_codes tables, the "biomequest" bucket,
// the biome_quest product tag, and ASSETS_BASE for the game <base> inject.
// =====================================================================
...
export const PRICE_BIOMEQUEST = process.env.STRIPE_PRICE_BIOMEQUEST;
export const WEBHOOK_SECRET = process.env.STRIPE_WEBHOOK_SECRET;
export const SITE_ORIGIN = process.env.SITE_ORIGIN || '';

// Where the game's relative assets (Cutscenes/, Images/, Soundtrack/,
// three.min.r128.js, bq_icon.png, …) live publicly. Defaults to the site
...
// Service-role Supabase client — server only, bypasses RLS. The Biome
...
// CORS: allow the live Biome Quest site + localhost during testing.
...
// Student codes like 'BIOME-7K2PX'. Deliberately avoids look-alike
...
export const BQ_BUCKET = 'biomequest';
export async function signAsset(db, path, seconds) {
  const { data, error } = await db.storage.from(BQ_BUCKET).createSignedUrl(path, seconds);
...
export async function sendEmail({ to, subject, html }) {
  ...
  const from = process.env.RESEND_FROM || 'Biome Quest <onboarding@resend.dev>';
  try {
...
export function questEmailHtml(token) {
  ...
  return '<div style="font-family:Georgia,serif;max-width:540px;margin:auto;color:#2a2520;line-height:1.6">' +
    '<h1 style="color:#5a7d3c">Your Biome Quest is ready &#127757;</h1>' +
    '<p>Your Biome Quest access is ready. Below is your <b>private teacher dashboard</b> — bookmark it; it\'s how you manage your 150 student codes.</p>' +
    ...
    '<ol style="margin:0;padding-left:20px">' +
      ...
      '<li>Track everyone on the Hall of Scientists, and reset free each year — no second purchase.</li>' +
    '</ol>' +
...
export function trialConfirmEmailHtml(link) {
  return '<div style="font-family:Georgia,serif;max-width:540px;margin:auto;color:#2a2520;line-height:1.6">' +
    '<h1 style="color:#5a7d3c">Start your free week of Biome Quest &#127757;</h1>' +
    '<p>Confirm your email to unlock <b>5 student seats free for 7 days</b> &mdash; the full game, no card required.</p>' +
    '<p><a href="' + link + '" style="display:inline-block;background:#6fa84f;color:#142008;font-weight:bold;padding:12px 22px;border-radius:6px;text-decoration:none">Start my free week</a></p>' +
...
export function trialWelcomeEmailHtml(token, trialEndsISO) {
  ...
  return '<div style="font-family:Georgia,serif;max-width:540px;margin:auto;color:#2a2520;line-height:1.6">' +
    '<h1 style="color:#5a7d3c">Your free week is live &#127757;</h1>' +
    '<p>You\'ve got <b>5 student seats</b> to try Biome Quest free' + (ends ? ' through <b>' + ends + '</b>' : '') + '. Below is your <b>private teacher dashboard</b> &mdash; bookmark it.</p>' +
    '<p><a href="' + dash + '" style="display:inline-block;background:#6fa84f;color:#142008;font-weight:bold;padding:12px 22px;border-radius:6px;text-decoration:none">Open my dashboard</a></p>' +
    '<p style="font-size:13px;color:#666;word-break:break-all">Or paste this link into your browser:<br>' + dash + '</p>' +
    '<hr style="border:none;border-top:1px solid #ddd;margin:18px 0">' +
    '<p style="margin:0 0 6px"><b>Getting started</b></p>' +
...
export async function topUpCodes(db, ownerId, target) {
  for (let attempt = 0; attempt < 50; attempt++) {
    const { count } = await db.from('bq_codes')
      .select('code', { count: 'exact', head: true }).eq('owner', ownerId);
    const have = count || 0;
    if (have >= target) break;
    const rows = genCodes(target - have).map(code => ({ code, owner: ownerId }));
    const { error } = await db.from('bq_codes').upsert(rows, { onConflict: 'code', ignoreDuplicates: true });
...
export async function provisionOwner(db, { email, source = 'purchase', stripe_customer = null, stripe_payment_intent = null, seats = FULL_SEATS, trialDays = null }) {
  ...
  const { data: owner, error } = await db.from('bq_owners')
  ...
  if (email) {
    try {
      if (trialDays) await sendEmail({ to: email, subject: 'Your free week of Biome Quest is ready', html: trialWelcomeEmailHtml(token, trial_ends) });
      else await sendEmail({ to: email, subject: 'Your Biome Quest dashboard + 150 codes', html: questEmailHtml(token) });
    } catch (e) {}
  }
  return token;
}
...
export async function upgradeOwnerToFull(db, ownerId, { stripe_customer = null, stripe_payment_intent = null } = {}) {
  ...
  await db.from('bq_owners').update(upd).eq('id', ownerId);
