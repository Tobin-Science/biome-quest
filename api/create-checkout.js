...
// Starts a Stripe Checkout for the one-time $19.99 Biome Quest purchase.
// No login required — Checkout collects the buyer's email, which the
// webhook uses to send them their private dashboard link.
// =====================================================================
import { stripe, PRICE_BIOMEQUEST, SITE_ORIGIN, adminDb, corsHeaders, json } from './_shared.js';
...
export async function POST(request) {
  ...
  try {
    ...
    const metadata = { product: 'biome_quest' };
    ...
    if (token) {
      const { data: owner } = await adminDb().from('bq_owners')
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      line_items: [{ price: PRICE_BIOMEQUEST, quantity: 1 }],
