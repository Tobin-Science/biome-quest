...
// When a Biome Quest purchase completes, create the owner (with a secret
...
export async function POST(request) {
  ...
  const isQuest = s.mode === 'payment' && s.metadata && s.metadata.product === 'biome_quest';
  if (!isQuest) return new Response('not a biome quest purchase', { status: 200 });
