...
//
// Biome Quest difference vs Science Quest: the game references many
// external assets (Cutscenes/, Images/, Soundtrack/, three.min.r128.js,
// bq_icon.png, …) by RELATIVE path. Since the page is served from
...
// =====================================================================
import { adminDb, BQ_BUCKET, ASSETS_BASE } from './_shared.js';
...
export async function GET(request) {
  try {
    ...
    if (!row.activated_at) {
      await db.from('bq_codes').update({ activated_at: new Date().toISOString() }).eq('code', code);
    }

    const { data: file, error: dErr } = await db.storage.from(BQ_BUCKET).download('game.html');
