...
export async function POST(request) {
  ...
  try {
    ...
    if (email) {
      const db = adminDb();
      const { data: owners } = await db.from('bq_owners')
        .select('access_token, email').ilike('email', email);
      for (const o of (owners || [])) {
        try { await sendEmail({ to: o.email, subject: 'Your Biome Quest dashboard link', html: questEmailHtml(o.access_token) }); } catch (e) {}
      }
    }
