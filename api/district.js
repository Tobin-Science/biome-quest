...
export async function POST(request) {
  ...
  try {
    ...
    if (kind === 'trial') {
      ...
      await sendEmail({ to: email, subject: 'Confirm your email to start your free week of Biome Quest', html: trialConfirmEmailHtml(link) });
    ...
    const html = '<div style="font-family:Georgia,serif;max-width:520px;margin:auto;color:#2a2520;line-height:1.6">' +
      '<h1 style="color:#5a7d3c">Activate your free Biome Quest &#127757;</h1>' +
      '<p>Cherokee County teachers get Biome Quest free. Click below to activate your access and generate your 150 student codes:</p>' +
    ...
    '</div>';
    await sendEmail({ to: email, subject: 'Activate your free Biome Quest access', html });
    return json({ ok: true }, 200, origin);
  } catch (e) {
    return json({ ok: true }, 200, origin);
  }
...
export async function GET(request) {
  try {
    ...
    if (ttoken) {
      ...
      const { data: existing } = await db.from('bq_owners')
