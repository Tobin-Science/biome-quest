...
(() => {
  ...
  function boot(runtime, doc = document) {
    ...
    // re-read the page to refine HTML — but fetch(file://) is CORS-blocked when opened as a local file, so skip it there
    (location.protocol === "file:" ? Promise.resolve("") : fetch(location.href).then((res) => res.ok ? res.text() : "")).then((t) => {
  ...
  var BABEL_URL = "babel.min.js";   // local (no unpkg CDN — works on locked school networks)
  ...
  // src/index.ts
  var REACT_URL = "react.production.min.js";   // local (no unpkg CDN — works on locked school networks)
  var REACT_SRI = "";
  var REACT_DOM_URL = "react-dom.production.min.js";   // local
  var REACT_DOM_SRI = "";
  ...
  function loadScript(src, integrity) {
    return new Promise((resolve2, reject) => {
      ...
      // no crossOrigin/integrity: a plain script tag loads under file:// (CORS would block a local file). Works offline.
      s.async = false;
      s.onload = () => resolve2();
